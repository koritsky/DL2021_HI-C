...
class computeInsulation(torch.nn.Module):
    def __init__(self, window_radius=10, deriv_size=10):
        ...
        self.di_pool = torch.nn.AvgPool2d(
            kernel_size=(2*window_radius+1), stride=1)  # 51
    ...
    def forward(self, x):
        ...
        right = torch.cat(
            [dv, torch.zeros(dv.shape[0], dv.shape[1], 2)], dim=2)
        band = ((left < 0) == torch.ones_like(left)) * \
            ((right > 0) == torch.ones_like(right))
        band = band[:, :, 2:-2]
        boundaries = []
        for i in range(0, band.shape[0]):
            cur_bound = torch.where(band[i, 0])[
                0]+self.window_radius+self.deriv_size
