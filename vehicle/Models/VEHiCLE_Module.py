...
class GAN_Model(pl.LightningModule):
    def __init__(self, with_vae=True, batch_size=1):
        ...
        self.mse = nn.L1Loss()
        self.vae_yaml = "vehicle/Weights/vehicle_vae_hparams.yaml"
        self.vae_weight = "vehicle/Weights/vehicle_vae_sk.ckpt"
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        if optimizer_idx == 0:
            ...
            if self.with_vae:
                ...
                total_loss_G = (self.tad_lambda * TAD_loss) + (self.vae_lambda * VAE_loss) + (
                    self.mse_lambda * MSE_loss) + (self.gan_lambda * GAN_loss)
            else:
                total_loss_G = (self.tad_lambda * TAD_loss) + \
                    (self.mse_lambda * MSE_loss) + (self.gan_lambda * GAN_loss)
            self.logger.experiment.log_metric(
                'train/total_loss_G', total_loss_G)
        ...
        if optimizer_idx == 1:
            ...
            acc_real = (pred_labels_real == labels_real).float(
            ).sum() / labels_real.shape[0]
            ...
            acc_fake = (pred_labels_fake == labels_fake).float(
            ).sum()/labels_fake.shape[0]
            ...
            self.logger.experiment.log_metric(
                'train/total_loss_D', total_loss_D)
            return total_loss_D
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if batch_idx == 5:
            self.logger.experiment.log_image(
                'val/target_img', self.get_img(data, target, output, MSE_loss))
    ...
    def on_validation_epoch_end(self):
        if self.is_sanity_check:
            self.exp_dir = self.trainer.logger.name + \
