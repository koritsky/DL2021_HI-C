    ...
    def log_images_epoch(self, outputs, mode="val"):
        ...
        for i in range(min(10, len(outputs))):
            ...
            for j in range(pred.shape[0]):
                img_pred = from_upper_triu(pred[j].cpu().numpy(), self.model.image_size, self.model.diagonal_offset)
        ...
        self.logger.experiment.add_image(f"{mode}_pred", grid_pred, self.current_epoch)
        self.logger.experiment.add_image(f"{mode}_target", grid_target, self.current_epoch)
        self.logger.experiment.add_image(f"{mode}_imgs", grid_both, self.current_epoch)
