...
class UpperTri(nn.Module):
    ...
    def forward(self, inputs):
        ...
        triu_index = torch.from_numpy(
            triu_tup[0] + seq_len*triu_tup[1]).to(inputs.device)
        triu_index = triu_index.unsqueeze(
            0).unsqueeze(1).repeat(b, output_dim, 1)
...
class StochasticShift(nn.Module):
    def __init__(self, shift_max=0, symmetric=True):
        ...
        if self.symmetric:
            self.augment_shifts = torch.arange(-self.shift_max,
                                               self.shift_max+1)
        else:
...
class SwitchReverseTriu(nn.Module):
    ...
    def forward(self, x, reverse):
        if reverse:
            ...
            mask_ld = ~mask_ut
            mat_indexes = mat_ut_indexes + \
                np.multiply(mask_ld, mat_ut_indexes.T)
            mat_rc_indexes = mat_indexes[::-1, ::-1]
            rc_ut_order = torch.from_numpy(
                mat_rc_indexes[ut_indexes]).to(x.device).type(torch.int64)
            rc_ut_order = rc_ut_order.unsqueeze(
                0).unsqueeze(1).repeat(bs, num, 1)
...
def get_indexes_embs(x, emb):
    ...
    triu_index = torch.from_numpy(
        triu_tup[0] + seq_len*triu_tup[1]).to(x.device)
    triu_index = triu_index.unsqueeze(
        0).unsqueeze(1).repeat(b, emb.shape[1], 1)
    emb_unroll = emb.reshape(-1, emb.shape[1],
                             seq_len**2).gather(-1, index=triu_index)
    emb_unroll = emb_unroll.transpose(1, 2).reshape(-1, emb_unroll.shape[1])
    triu_tup = np.stack([triu_tup[0], triu_tup[1]])
    inds = torch.from_numpy(np.concatenate(
        [i*seq_len+triu_tup for i in range(b)], axis=1)).to(x.device)
...
def get_2d_from_upper_tri_only(x):
    b, n, s, _ = x.shape
    mask_tri = (torch.tril(torch.ones(s, s)) == 1).unsqueeze(
        0).unsqueeze(1).repeat(b, n, 1, 1)
    mask_diag = torch.eye(s, dtype=torch.bool).unsqueeze(
        0).unsqueeze(1).repeat(b, n, 1, 1)
