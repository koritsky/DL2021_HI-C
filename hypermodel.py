...
class HyperModel(pl.LightningModule):
    ...
    def log_pictures(self, akita_img, vehicle_img, final_img, high_img, phase):
        grid = utils.make_grid(self.get_colors(final_img), nrow=2)
        self.logger.experiment.log_image('{}/final_img'.format(phase), grid, self.current_epoch)
        
        grid = utils.make_grid(self.get_colors(akita_img), nrow=2)
        self.logger.experiment.log_image('{}/akita_img'.format(phase), grid, self.current_epoch)
        
        grid = utils.make_grid(self.get_colors(vehicle_img), nrow=2)
        self.logger.experiment.log_image('{}/vehicle_img'.format(phase), grid, self.current_epoch)

        grid = utils.make_grid(self.get_colors(high_img), nrow=2)
        self.logger.experiment.log_image('{}/high_img'.format(phase), grid, self.current_epoch)
    ...
    def training_epoch_end(self, outputs):
        ...
        ###logging####
        self.logger.experiment.log_metric('train/loss', averaged_loss)
        self.logger.experiment.log_metric('train/akita_loss', averaged_akita_loss)
        self.logger.experiment.log_metric('train/vehicle_loss', averaged_vehicle_loss)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        akita_metrics = self.calculate_metrics(akita_output.detach(), high_img)
        vehicle_metrics = self.calculate_metrics(vehicle_output.detach(), high_img)
        final_metrics = self.calculate_metrics(output.detach(), high_img)
    ...
    def validation_epoch_end(self, outputs):
        ...
        ###logging####
        self.logger.experiment.log_metric('val/loss', averaged_loss)
        self.logger.experiment.log_metric('val/akita_loss', averaged_akita_loss)
        self.logger.experiment.log_metric('val/vehicle_loss', averaged_vehicle_loss)
